text \<open> Version 0.4, last changed 2018-02-27 
  ...
  such as about the maximum and minimum of linear approximation. \<close>
...
text \<open> The nearest rank method assumes a partial function (concretely, a function with domain type 
  ...
  staying half the distance (i.e., 1/10) away from the left and right border, that is, you get 
...
definition linear_approximation :: "(real \<Rightarrow> real option) \<Rightarrow> real \<Rightarrow> real"
where
  "linear_approximation p x = (let (x1, x2) = neighbors p x;
...
text \<open> If the domain of a partial function p is finite and an element in the defined interval of p then
...
lemma upper_bound:
  assumes "finite (dom p)"
  assumes "x \<in> defined_interval_of p"
  shows "linear_approximation p x \<le> Max (ran p)"
proof -
  have "finite (ran p)"
  ...
  from this have "linear_approximation p x = linear (x1, y1) (x2, y2) x"
    unfolding linear_approximation_def by simp
  also have "\<dots> \<le> max y1 y2"
...
lemma lower_bound:
  assumes "finite (dom p)"
  assumes "x \<in> defined_interval_of p"
  shows "Min (ran p) \<le> linear_approximation p x"
proof -
  have "finite (ran p)"
    using assms(1) by (simp add: finite_ran)
  from assms obtain x1 y1 x2 y2 where
  ...
  also have "\<dots> = linear_approximation p x"
    unfolding linear_approximation_def
...
text \<open> If a partial function has finite domain and is monotonically increasing for the values where
  it is defined, then its linear approximation is monotonically increasing.\<close>
lemma linear_approximation_monotonic:
  assumes "finite (dom p)"
  assumes "x \<in> defined_interval_of p" "x' \<in> defined_interval_of p"
  assumes "\<forall>x x' y y'. x \<le> x' \<and> p x = Some y \<and> p x' = Some y' \<longrightarrow> y \<le> y'"
  assumes "x \<le> x'"
  shows "linear_approximation p x \<le> linear_approximation p x'"
proof -
  ...
  from neighbors_x have "linear_approximation p x = linear (x1, y1) (x2, y2) x"
    unfolding linear_approximation_def by simp
  ...
  qed
  also have "linear (x1', y1') (x2', y2') x' = linear_approximation p x'"
    unfolding linear_approximation_def
...
lemma neighbors_scale_invariant:
  shows "neighbors (\<lambda>x. map_option (\<lambda>y. (c * y)) (p x)) x = neighbors p x"
  using neighbors_def by (simp add: domIff)

text \<open> Linear approximation of a partial function is linear with respect to multiplication. \<close>
lemma linear_approximation_scale:
  assumes "finite (dom p)"
  assumes "x \<in> defined_interval_of p"
  shows "linear_approximation (\<lambda>x. map_option (\<lambda>y. c * y) (p x)) x = c * linear_approximation p x"
proof -
  ...
  have "linear_approximation (\<lambda>x. map_option (\<lambda>y. c * y) (p x)) x = linear (x1, c * the (p x1)) (x2, c * the (p x2)) x"
    using \<open>neighbors p x = (x1, x2)\<close> \<open>p x1 = Some y1\<close> \<open>p x2 = Some y2\<close>
    unfolding linear_approximation_def by (simp add: neighbors_scale_invariant)
  also have "\<dots> = c * linear_approximation p x"
    unfolding linear_approximation_def
...
lemma empty_case:
  ...
  shows "\<exists>K. \<bar>linear_approximation p x' - linear_approximation p x\<bar> \<le> K * \<bar>x' - x\<bar>"
proof cases
  assume "x = x'"
...
next
  ...
  proof (rule four_cases[of x p x'])
    ...
    from 1 this show ?thesis
      unfolding linear_approximation_def apply simp
  ...
  next
    ...
    have "linear_approximation p x = linear (x1, y1) (x2, y2) x"
      using \<open>neighbors p x = (x1, x2)\<close> \<open>p x1 = _\<close> \<open>p x2 = _\<close>
      unfolding linear_approximation_def by simp
    moreover have "linear_approximation p x' = linear (x1, y1) (x2, y2) x'"
    proof -
      from n \<open>p x1 = _\<close> \<open>p x2 = _\<close> show ?thesis
        unfolding linear_approximation_def apply auto done
    qed
    ultimately show "\<exists>K. \<bar>linear_approximation p x' - linear_approximation p x\<bar> \<le> K * \<bar>x' - x\<bar>"
...
lemma linear_approximation_Lipschitz:
  assumes "finite (dom p)"
  assumes "x \<in> defined_interval_of p" "x' \<in> defined_interval_of p"
  shows "\<exists>K. abs (linear_approximation p x' - linear_approximation p x) \<le> K * abs (x' - x)"
proof -
  { fix x x' :: real
  ...
  { fix A
    have "finite A \<Longrightarrow> x \<le> x' \<Longrightarrow> x \<in> defined_interval_of p \<Longrightarrow> {x''. \<exists>y. p x'' = Some y \<and> x < x'' \<and> x'' \<le> x'} = A \<longrightarrow>
      (\<exists>K. abs (linear_approximation p x' - linear_approximation p x) \<le> K * abs (x' - x))"
    proof (induct A arbitrary: x rule: finite_Min_induct)
      case empty
      {
        ...
        from this have "\<exists>K. \<bar>linear_approximation p x' - linear_approximation p x\<bar> \<le> K * \<bar>x' - x\<bar>"
    ...
    next
      case (insert A a)
      {
        ...
        (* possible improvement: replace linear_approximation p by an local abbreviation for shorter proofs *)
        from insert.hyps(2) \<open>a \<le> x'\<close> \<open>a \<in> _\<close> this obtain K where
          1: "\<bar>linear_approximation p x' - linear_approximation p a\<bar> \<le> K * \<bar>x' - a\<bar>" by auto
        ...
        from this obtain K' where 2: "\<bar>linear_approximation p a - linear_approximation p x\<bar> \<le> K' * \<bar>a - x\<bar>"
          using empty_case \<open>x < a\<close> \<open>finite (dom p)\<close> \<open>x \<in> _\<close> \<open>a \<in> _\<close> by fastforce
        have "\<bar>linear_approximation p x' - linear_approximation p x\<bar> \<le>
          \<bar>linear_approximation p x' - linear_approximation p a\<bar> + \<bar>linear_approximation p a - linear_approximation p x\<bar>"
          by auto
        ...
        qed
        finally have "\<exists>K. \<bar>linear_approximation p x' - linear_approximation p x\<bar> \<le> K * \<bar>x' - x\<bar>" by auto
      }
      then show ?case by auto
    qed
  }
