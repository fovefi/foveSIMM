...
lemma diff_list_non_negative:
   assumes "sorted (x#y#xs:: (real list))"
         shows   "(diff_list (x#y#xs)) ! 0 \<ge> 0"
proof -
  have "(diff_list (x#y#xs))!0 = y - x" using diff_list.simps
    by (metis neq_Nil_conv nth_Cons_0)
  then show ?thesis
    using assms by auto
qed
...
lemma diff_list_max_non_negative:
   assumes "sorted (xs:: (real list))"
           "length xs > 1"
   shows   "Max (set (diff_list xs)) \<ge> 0"
   proof -
   have f1: "(diff_list xs)! 0 \<ge> 0" using diff_list_non_negative 
        using assms(1) assms(2)
        by (metis diff_list.elims diff_list_length length_greater_0_conv less_imp_le_nat zero_less_diff)
   have f2: "length (diff_list xs) > 0" using diff_list_length assms(2) zero_less_diff by auto
   have f3: "Max (set (diff_list xs)) \<ge> (diff_list xs)!0" 
     using assms(2) f2 by simp
   show ?thesis using f1 f3 by linarith
qed

text \<open>The difference list operation commutes with linear scaling. \<close>
lemma difflist_map:
  assumes "length (ys :: (real list)) > 1"
  assumes "c > 0"
  assumes "sorted ys"
  shows "diff_list (map (\<lambda> z. c*z) ys) = (map (\<lambda> z. c* z) (diff_list ys))"
proof (induct ys)
  case Nil
  then show ?case by simp
next
  case (Cons a ys)
  have "\<forall> xs x1 x2. map (\<lambda> z. c* z) ((x2 - x1) # xs) = (c * (x2 - x1)) # (map (\<lambda> z. c* z) xs)" by simp
  obtain yhd where "yhd = hd ys" by auto
  obtain ytl where "ytl = tl ys" by auto
  then have "\<forall> y::real. diff_list (y#yhd#ytl) = (yhd - y) # (diff_list ys)" 
  proof -
    show ?thesis
      by (metis \<open>yhd = hd ys\<close> \<open>ytl = tl ys\<close> diff_list.simps(1) diff_list.simps(2) diff_list.simps(3) diff_list.simps(4) list.exhaust_sel list.sel(2))
  qed 
  have a: "\<forall> y::real. diff_list (map (\<lambda> z. c*z) (y#yhd#ytl)) = diff_list ((c*y)#(c*yhd)#(map (\<lambda> z. c*z) ytl))" by simp
  then have "\<forall> y::real. diff_list (map (\<lambda> z. c*z) (y#yhd#ytl)) = (c * yhd - c * y) # (map (\<lambda> z. c* z) (diff_list ys))" 
    using Cons.hyps Cons_eq_map_conv \<open>\<forall>y. diff_list (y # yhd # ytl) = (yhd - y) # diff_list ys\<close> 
    \<open>yhd = hd ys\<close> \<open>ytl = tl ys\<close> diff_list.elims diff_list.simps(1) diff_list.simps(3) list.collapse 
    list.discI list.inject list.map_disc_iff list.sel(2) list.sel(3) map_tl 
  proof -
    { fix rr :: real
      have ff1: "\<forall>r ra rs. diff_list (r # ra # rs) = (ra - r) # diff_list (ra # rs) \<or> rs = []"
        by (metis (full_types) diff_list.simps(2) list.exhaust)
      have ff2: "\<forall>f. (f yhd::real) # map f (tl ys) = map f ys \<or> ys = []"
        by (metis (no_types) \<open>yhd = hd ys\<close> list.exhaust_sel list.simps(9))
    { assume "ys \<noteq> []"
      then have "ys \<noteq> [yhd] \<and> ys \<noteq> [] \<or> diff_list (c * rr # c * yhd # map ((*) c) (tl ys)) = c * (yhd - rr) # diff_list (map ((*) c) ys)"
        by (simp add: linordered_field_class.sign_simps(19))
      then have "diff_list (c * rr # c * yhd # map ((*) c) (tl ys)) = c * (yhd - rr) # diff_list (map ((*) c) ys)"
        using ff2 ff1 by (metis (no_types) Nil_is_map_conv \<open>yhd = hd ys\<close> linordered_field_class.sign_simps(19) list.exhaust_sel) }
    then have "diff_list (map ((*) c) (rr # yhd # ytl)) = (c * yhd - c * rr) # map ((*) c) (diff_list ys)"
      using Cons.hyps \<open>ytl = tl ys\<close> linordered_field_class.sign_simps(19) by fastforce }
  then show ?thesis
    by blast
qed
  then have "\<forall>y. diff_list (map (\<lambda> z. c*z) (y#ys)) = (map (\<lambda> z. c* z) (diff_list (y#ys)))" using a 
    by (metis (no_types, hide_lams) \<open>\<forall>y. diff_list (y # yhd # ytl) = (yhd - y) # diff_list ys\<close> 
    \<open>yhd = hd ys\<close> \<open>ytl = tl ys\<close> diff_list.simps(4) linordered_field_class.sign_simps(19) list.exhaust_sel list.simps(8) list.simps(9))
  then show ?case using Cons.hyps a by blast
qed


text \<open> The extrapolation to below is done by taking then MAXIMAL slope as between the any two points 
  and extending it to 0. E.g., for a list [3,1,4,9,6], we first sort the list to
  [1,3,4,6,9] and compute the maximal slope as the maximal difference between two adjacent elements divided
  by 0.2, that is, max{2,1,2,3}/0.2 = 3/0.2 = 15. The extrapolation is  the value of the straight line
  f(x) = f(x_0) + slope*(x-x_0) at position 0, that is f(0) = 1 + 15 * (0 - 0.1) = 1 - 0.1 * 15 = -0.5. \<close>
definition extrapolation_0 :: "real list \<Rightarrow> real"
  where
  "extrapolation_0 ys = ys!0 - (Max (set (diff_list ys))) / 2"

value "diff_list [1::real,3,4,6,9]"
value "Max (set (diff_list [1::real,3,4,6,9]))"
value "extrapolation_0 [1::real,3,4,6,9]"
...
proof -
  ...
  then show ?thesis using extrapolation_0_def
    using assms(1) assms(2) diff_list_max_non_negative by auto
qed   
...
proof -
  ...
  have small: "extrapolation_0 ys \<le> ys!0"
    using assms(1) assms(2) extrapolation_0_def sorted_nth_mono
...
proof -
  ...
  then have "ys!1 \<le> 
    ...
    by arith
  then show ?thesis
    using \<open>ys ! 0 \<le> ys ! 1\<close> extrapolation_0_def
    by (metis Suc_eq_plus1 Suc_leI assms(1) assms(2) extrapolation_0_small one_add_one)
qed
...
proof -
  ...
  then have "ys!((length ys) -1) \<le> 
    ...
    by arith
  then show ?thesis
    using assms(1) assms(2) extrapolation_0_def extrapolation_0_smaller_first extrapolation_1_def by auto
qed
...
proof -
  ...
  then have "ys!((length ys) -1) \<le> ys!((length ys) -2) + 
    ...
    by linarith
  then have "ys!((length ys) -1) \<le> extrapolation_1 ys" using extrapolation_1_def
    using \<open>extrapolation_0 ys \<le> ys ! 0\<close> extrapolation_0_def by auto
  then have "1 - 1 / (2 * length ys) < x \<and> x \<le> 1 \<longrightarrow> ?thesis"
    by (smt List.finite_set Min_le \<open>extrapolation_0 ys \<le> Min (set ys)\<close> assms(1) assms(2)
        diff_less le_less_trans le_numeral_extra(1) less_numeral_extra(1) linear_lower_bound nth_mem percentile_alt3)
...
proof -
  ...
  then show ?thesis using percentile_inner_def b c
    by (simp add: \<open>sort (map ((*) c) ys) = map ((*) c) (sort ys)\<close> assms(4) sorted_sort_id)
qed

text \<open> The percentile function scales by constant (positive) factors.\<close>


text \<open>For a finite set partitioned into to non-empty subsets the maximum can be determined by the
      maximum of the maxima of the subsets.\<close>
lemma max_union:
  assumes "finite A"
  assumes "(S1 :: real set) \<subset> (A :: real set)"
  assumes "(S2 :: real set) \<subset> (A :: real set)"
  assumes "S1 \<noteq> {}"
  assumes "S2 \<noteq> {}"
  assumes "S1 \<union> S2 = A" 
  shows "Max A = max (Max S1) (Max S2)"
  using Max.union assms(1) assms(4) assms(5) assms(6) by blast

text \<open>The maximum of the set corresponding to a non-empty list of numbers commutes with the multiplication
      by a positive number.\<close>

lemma max_scale:
  assumes "(c::real) > 0"
  shows "Max (set (map (\<lambda> z. c* z) (y#ys))) = c * (Max (set (y#ys)))"
proof (induct ys)
  print_cases
  case Nil
  have "Max (set [y]) = y" by auto
  have "set (map (\<lambda> z. c * z) [y]) = {c*y}" by auto
then show ?case
  by simp
next
  case (Cons a ys)
  {assume "ys \<noteq> []"
  have "\<forall> x. set (map (\<lambda> z. c * z) (y#x#ys)) = {c*y} \<union> {c*x} \<union> (set (map (\<lambda> z. c * z) ys))" by auto
  then have a: "\<forall> x. Max (set (map (\<lambda> z. c * z) (y#x#ys))) = Max ({c*y} \<union> {c*x} \<union> (set (map (\<lambda> z. c * z) ys)))" by auto
  have b: "set (map (\<lambda> z. c * z) (y#x#[])) \<noteq> {}" by blast
  then have c: "set (map (\<lambda> z. c * z) ys) \<noteq> {}" using \<open>ys \<noteq> []\<close> by blast
  then have d: "\<forall> x. Max (set (map (\<lambda> z. c * z) (y#x#ys))) = 
             max (Max (set (map (\<lambda> z. c * z) (y#x#[])))) (Max (set (map (\<lambda> z. c * z) ys)))" using 
                  max_union[of "set (map (\<lambda> z. c * z) (y#x#[]))" "set (map (\<lambda> z. c * z) (y#x#ys))" "set (map (\<lambda> z. c * z) ys)"]  assms b by force
  have "\<forall>x. Max (set (map (\<lambda> z. c * z) (y#x#[]))) = Max ({c*y} \<union> {c*x})" by auto
  then have "\<forall>x. Max (set (map (\<lambda> z. c * z) (y#x#[]))) = c * (Max ({y} \<union> {x}))" by (simp add: assms max_def) 
  then have "\<forall>x. max (Max (set (map (\<lambda> z. c * z) (y#x#[])))) (Max (set (map (\<lambda> z. c * z) ys))) = 
             max (c * (Max ({y} \<union> {x})))  (Max (set (map (\<lambda> z. c * z) ys)))" by auto
  then have "\<forall> x. Max (set (map (\<lambda> z. c * z) (y#x#ys))) = 
             max (c * (Max ({y} \<union> {x}))) (Max (set (map (\<lambda> z. c * z) ys)))" using d by simp
  then have "\<forall> x. Max (set (map (\<lambda> z. c * z) (y#x#ys))) = max (c * (Max ({y} \<union> {x})))  (c * (Max (set ys)))" using Cons.hyps
       a List.finite_set Max.union Max_insert Max_singleton assms(1) 
    empty_not_insert empty_set finite_insert insert_absorb2 list.map_disc_iff list.simps(15) 
    mult_less_cancel_left_pos set_empty set_map sup_max sup_real_def
  proof -
    { fix rr :: real
      have ff1: "\<forall>r. Max (set (map ((*) c) (y # r # ys))) = max (c * Max {y, r}) (Max (set (map ((*) c) ys)))"
        by (metis (full_types) \<open>\<forall>x. Max (set (map ((*) c) (y # x # ys))) = max (c * Max ({y} \<union> {x})) (Max (set (map ((*) c) ys)))\<close> insert_is_Un)
      have ff2: "\<forall>r ra. Max {ra::real, r} = max ra r"
        by force
      have ff3: "max (c * y) (Max (set (map ((*) c) ys))) = c * Max (insert y (set ys))"
        using ff1 by (metis (no_types) Cons.hyps \<open>\<And>x A. insert x (insert x A) = insert x A\<close> \<open>\<And>x. Max {x} = x\<close> \<open>\<And>xs f. set (map f xs) = f ` set xs\<close> list.set(2))
      have ff4: "\<forall>r ra rb. max ((rb::real) * ra) (rb * r) = rb * max ra r \<or> rb < 0"
        by (simp add: max_mult_distrib_left)
      have ff5: "\<not> c < 0"
        using \<open>0 < c\<close> by auto
      have ff6: "\<forall>r ra rb. max (rb::real) (max ra r) = max ra (max rb r)"
        by (metis (no_types) sup_left_commute sup_max)
      have ff7: "\<forall>r ra rb. max (max (rb::real) ra) r = max rb (max ra r)"
        by (metis sup_assoc sup_max)
      have "\<forall>r. max (r::real) r = r"
        by auto
      then have ff8: "\<forall>r ra. max (ra::real) (max r ra) = max r ra"
        using ff6 by presburger
    have ff9: "\<forall>r. max (c * max y r) (Max (set (map ((*) c) ys))) = max (c * r) (c * Max (insert y (set ys)))"
      using ff3 ff1 d by fastforce
    have ff10: "\<forall>r ra. max (ra::real) (max ra r) = max ra r"
      by fastforce
    have ff11: "\<forall>r rs. Max (insert (r::real) (set rs)) = max r (Max (set rs)) \<or> set rs = {}"
      by (meson \<open>\<And>x A. \<lbrakk>finite A; A \<noteq> {}\<rbrakk> \<Longrightarrow> Max (insert x A) = max x (Max A)\<close> \<open>\<And>xs. finite (set xs)\<close>)
    have ff12: "max (c * Max (insert y (set ys))) (Max (set (map ((*) c) ys))) = c * Max (insert y (set ys))"
      using ff3 by linarith
    have "set ys \<noteq> {} \<and> c \<noteq> 0"
      using \<open>0 < c\<close> c by fastforce
    moreover
    { assume "(set ys \<noteq> {} \<and> c \<noteq> 0) \<and> c * Max (insert y (set ys)) = max (c * y) (c * Max (set ys))"
      moreover
      { assume "(max y rr \<noteq> y \<and> set ys \<noteq> {}) \<and> c \<noteq> 0"
        moreover
        { assume "(Max (set ys) \<noteq> y \<and> set ys \<noteq> {}) \<and> c \<noteq> 0"
          moreover
          { assume "(Max (insert y (set ys)) \<noteq> Max (set ys) \<and> set ys \<noteq> {}) \<and> c \<noteq> 0"
            then have "c * Max (set ys) \<noteq> c * y \<and> set ys \<noteq> {} \<or> (\<exists>r. ((Max (insert y (set ys)) \<noteq> r \<and> set ys \<noteq> {}) \<and> c \<noteq> 0) \<and> c * y = c * r)"
              by metis
            moreover
            { assume "c * Max (set ys) \<noteq> c * y \<and> set ys \<noteq> {}"
              moreover
              { assume "c * max y (Max (set ys)) \<noteq> c * Max (set ys) \<and> set ys \<noteq> {}"
  moreover
                { assume "(c * max y (Max (set ys)) \<noteq> c * Max (set ys) \<and> set ys \<noteq> {}) \<and> Max (set (map ((*) c) ys)) = c * max y (Max (set ys))"
                  moreover
                  { assume "c * max y (max (Max (set ys)) rr) \<noteq> c * Max (set ys) \<and> c * max y (max (Max (set ys)) rr) = Max (set (map ((*) c) ys))"
                    then have "max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (c * Max (set ys)) (c * max y rr) = Max (set (map ((*) c) ys))"
                      using ff6 ff5 ff4 by metis
  then have "max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
                      by (metis (no_types)) }
                  ultimately have "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
                    by simp }
                moreover
                { assume "c * max y (Max (set ys)) \<noteq> c * Max (set ys) \<and> Max (set (map ((*) c) ys)) \<noteq> c * Max (insert y (set ys))"
                  moreover
                  { assume "c * max y (max (Max (set ys)) rr) \<noteq> c * Max (set ys) \<and> Max (set (map ((*) c) ys)) \<noteq> c * Max (insert y (set ys))"
                    then have "max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> Max (set (map ((*) c) ys)) \<noteq> c * Max (insert y (set ys))"
                      using ff6 ff5 ff4 by metis
                    then have "max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys))"
                      using ff12 ff9 ff8 by metis }
                  ultimately have "max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys))"
                    using ff12 ff10 by (metis (no_types)) }
                ultimately have "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (c * Max (set ys)) (c * max y rr) \<noteq> c * Max (set ys) \<and> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
                  using ff11 by (metis (no_types))
                then have "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
                  by (meson max_def) }
              ultimately have "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
                by (metis (no_types)) }
            ultimately have "(\<exists>r. ((Max (insert y (set ys)) \<noteq> r \<and> set ys \<noteq> {}) \<and> c \<noteq> 0) \<and> c * y = c * r) \<or> max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
              by fastforce }
          ultimately have "(\<exists>r. ((Max (insert y (set ys)) \<noteq> r \<and> set ys \<noteq> {}) \<and> c \<noteq> 0) \<and> c * y = c * r) \<or> max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {} \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
            by force }
        moreover
        { assume "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> set ys \<noteq> {}"
          moreover
          { assume "max (Max (set ys)) rr \<noteq> Max (set ys) \<and> c * max y (max (Max (set ys)) rr) = Max (set (map ((*) c) ys))"
            then have "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
              using ff6 ff5 ff4 by metis }
          ultimately have "max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys) \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
            using ff11 ff7 ff5 ff4 by (metis (no_types)) }
        moreover
        { assume "max (c * Max (insert y (set ys))) (c * rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys)"
          then have "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys)"
            using ff9 by (simp add: max.commute) }
        moreover
        { assume "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (Max (set ys)) rr \<noteq> Max (set ys)"
          then have "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> c * max y (max (Max (set ys)) rr) = c * max y rr"
            by (metis max_def)
          then have "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr"
            using ff6 ff5 ff4 by metis }
        moreover
        { assume "max (Max (set (map ((*) c) ys))) (c * max y rr) \<noteq> Max (set (map ((*) c) ys)) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y rr"
          then have "max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
            by (metis max_def) }
        ultimately have "(\<exists>r. ((Max (insert y (set ys)) \<noteq> r \<and> set ys \<noteq> {}) \<and> c \<noteq> 0) \<and> c * y = c * r) \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
          by fastforce }
      ultimately have "(\<exists>r. ((Max (insert y (set ys)) \<noteq> r \<and> set ys \<noteq> {}) \<and> c \<noteq> 0) \<and> c * y = c * r) \<or> c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {} \<or> max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
        using ff3 by (metis max.commute) }
    moreover
    { assume "c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {}"
      then have "(c * max y (Max (set ys)) \<noteq> c * y \<and> set ys \<noteq> {}) \<and> max (c * Max (set ys)) (c * max y rr) = c * max y (max (Max (set ys)) rr)"
        using ff6 ff5 ff4 by (metis (no_types))
      then have "c * Max (insert y (set ys)) \<noteq> c * y \<and> c * max y (max (Max (insert y (set ys))) rr) = max (c * Max (set ys)) (c * max y rr)"
        using ff7 by simp
      then have "max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
        using ff6 ff5 ff4 ff3 by (metis max_def) }
    ultimately have "max (Max (set (map ((*) c) ys))) (c * max y rr) = max (c * Max (set ys)) (c * max y rr)"
      using ff11 ff9 ff3 by (smt mult_left_cancel) 
    then have "Max (set (map ((*) c) (y # rr # ys))) = max (c * Max ({y} \<union> {rr})) (c * Max (set ys))"
      using ff2 ff1 by (metis (no_types) insert_is_Un max.commute) }
  then show ?thesis
    by blast
qed
  have "\<forall> a::real. \<forall> b::real. max (c*a) (c*b) = c * (max a b)"
    by (smt assms mult_less_cancel_left)
  then have "\<forall> x::real. max (c * (Max ({y} \<union> {x})))  (c * (Max (set ys))) = c * max (Max ({y} \<union> {x})) (Max (set ys))" using assms by blast
have f1: "\<And>r. c * max (Max (set ys)) (Max {y, r}) = Max (((*) c) ` (insert y (insert r (set ys))))"
by (metis (no_types) \<open>\<forall>x. Max (set (map ((*) c) (y # x # ys))) = max (c * Max ({y} \<union> {x})) (c * Max (set ys))\<close> \<open>\<forall>x. max (c * Max ({y} \<union> {x})) (c * Max (set ys)) = c * max (Max ({y} \<union> {x})) (Max (set ys))\<close> insert_is_Un list.simps(15) max.commute set_map)
  have f2: "\<And>r ra rb. max (r::real) (max ra rb) = max ra (max r rb)"
    by (metis (no_types) sup_left_commute sup_max)
  then have f3: "\<And>r ra rb. max (r::real) (max ra rb) = max rb (max r ra)"
    by (simp add: max.commute)
  have "c * max (Max {y}) (Max (set ys)) = c * Max (insert y (set ys))"
    using f1 by (metis (no_types) Cons.hyps insert_absorb2 list.simps(15) max.commute set_map)
  then have "\<And>r. c * max (max (Max {y}) (Max (set ys))) r = c * max (Max (insert y (set ys))) r"
    by auto
  then have f4: "\<And>r. c * max (Max {y}) (max (Max (set ys)) r) = c * max (Max (insert y (set ys))) r"
    using f2 by (simp add: max.commute)
  have f5: "\<And>r. finite {r::real}"
    by blast
  have "c * Max (insert a (insert y (set ys))) = c * max (Max (insert y (set ys))) a"
    by (simp add: max.commute)
  then have "Max (((*) c) ` (insert a (insert y (set ys)))) = c * Max (insert a (insert y (set ys)))"
    using f5 f4 f3 f1 by (metis (no_types) Max_insert empty_not_insert insert_commute)
}
  then have "ys \<noteq> [] \<longrightarrow> ?case" by (simp add: insert_commute)
  {  assume "ys = []"
    then have ?case by (simp add: assms max_def)
  }
  then have "ys = [] \<longrightarrow> ?case" by blast
  then show ?case
    using \<open>ys \<noteq> [] \<longrightarrow> Max (set (map ((*) c) (y # a # ys))) = c * Max (set (y # a # ys))\<close> by blast
qed

text \<open>The percentile function scales linearly under multiplication with a positive number\<close>
...
proof -
  ...
  have "set (diff_list (map (\<lambda> z. c*z) ys)) = set (map (\<lambda> z. c* z) (diff_list ys))" using difflist_map
    using assms(1) assms(2) assms(3) by auto
  then have max_lin: "Max (set (diff_list (map (\<lambda> z. c*z) ys))) = c * (Max (set (diff_list ys)))" using max_scale
    by (metis assms(1) assms(2)  diff_list_length length_0_conv less_imp_le_nat list.exhaust_sel neq0_conv zero_less_diff) 
  then have "extrapolation_0 (map (\<lambda> z. c *z) ys) = 
        c * (extrapolation_0 ys)"  using extrapolation_0_def
    by (metis assms(1) le_less_trans le_numeral_extra(1) linordered_field_class.sign_simps(19) nth_map times_divide_eq_right)
  ...
  then have "extrapolation_1 (map (\<lambda> z. c *z) ys) = 
        c * (extrapolation_1 ys)"  using extrapolation_1_def max_lin
    by (metis (no_types, hide_lams) assms(1) diff_less le_less_trans length_map less_numeral_extra(1) 
        linordered_field_class.sign_simps(18) nth_map times_divide_eq_right zero_order(1))
