...
(* Version 0.1, started 2016-07-21, last changed 2017-04-02 *)
...
lemma member_max:
  ...
  apply (induct xs)
  apply simp
   by (metis Percentile2.member.simps(2) maximum.simps(1) maximum.simps(2) member.elims(3))
...
proof -
  ...
  have f9: "maximum (x # xsa) = x \<or> \<not> maximum (sort xsa) \<le> x"
    using f4 f2 by (metis Percentile2.member.simps(2) list.distinct(1) member.elims(2) member_max)
...
lemma maximum_lemma:
   ...
   apply (induction xs)
   apply (smt Percentile2.member.simps(1) Percentile2.member.simps(2) maximum.simps(1))
   by (smt [[smt_timeout = 10]] Percentile2.member.simps(2) list.inject maximum.simps(1) 
...
assumes "i\<le>j"
        "sorted xs"
shows "\<forall>i.\<forall>j.(i< length xs \<and> j < length xs) \<Longrightarrow> (xs!i \<le> xs!j)"
apply (induct xs)
apply simp by blast 
...
(* In a sorted list all elements are small than or equal to the last element. *)
...
(* In this section we define the difference list of an existing list of real numbers. 
 * For one-element lists we define it as the empty list. Note, the difference list is
 * a discrete analogon to the derivatives of a continuous function
 *)
...
proof -
  ...
  ultimately have "member ((x0 # x1 # a # xs) ! i - (x0 # x1 # a # xs) ! (i - 1)) 
                          ((x1 - x0) # diff_list (x1 # a # xs))"
    using f1 by (metis (no_types) Percentile2.member.simps(2) Suc_eq_plus1 hyp.prems(2) 
...
(* The maximum of the difference list of a sorted list is not negative.*)
...
(* In this section we show some properties of the ceiling function as well as a linear
 * interpolation property of the real numbers.
 *)
...
(*lemma monotonicity_addition:
  shows "0 \<le> (n ::nat) \<longrightarrow> ((\<forall>i. ((f i) :: real) \<le> 0) \<longrightarrow> (\<Sum> i \<in> {0..n}. (f i)) \<le> 0)"
  apply (induct n)
  apply auto
  by smt*)
...
(* In this chapter we define the percentile function, first in a form which is very close to the 
 * Java implementation in the OpenSimm implementation. This definition has the problem that it
 * is underspecified for confidence values close to 1. Accordingly, the Java implementation 
...
definition percentile_java :: "(real list) \<Rightarrow> real \<Rightarrow> real" 
  ...
  "percentile_java values level =
    (let  (size :: real) = (real (length values));
           ...
           (lowerValue :: real)  = sorted ! (nat (i-1));
           (upperValue :: real)  = sorted ! (nat i)
    in lowerValue + (level - lower) * (upperValue - lowerValue) / (upper - lower))"
...
definition percentile_sorted_below ::  "(real list) \<Rightarrow> real \<Rightarrow> real" where
  "percentile_sorted_below sorted_values level =
     (let (size :: real) = (real (length sorted_values));
           ...
           (lowerValue :: real)  = sorted_values ! (nat (i-1));
           (upperValue :: real)  = sorted_values ! (nat i);
           (\<alpha> :: real) = (level - lower) * size
    in lowerValue +  \<alpha> * (upperValue - lowerValue))"
...
definition percentile_sorted_above :: "(real list) \<Rightarrow> real \<Rightarrow> real" where
  "percentile_sorted_above sorted_values level =
      (let  (upperValue :: real)  = sorted_values ! ((length sorted_values) - 1);
            (\<alpha> :: real) = (maximum (diff_list sorted_values)) * (real (length sorted_values));
            (p_bar :: real) = 1.0 - 1.0/2.0/(real (length sorted_values))
      in upperValue +  \<alpha> * (level - p_bar))"

(* 
definition percentile_sorted_above_pessimistic 
      \<alpha> to be the maximum of the historic slope.
*)

definition percentile_sorted :: "(real list) \<Rightarrow> real \<Rightarrow> real" where
  "percentile_sorted sorted_values level =
    (let (size :: real) = (real (length sorted_values))
      in (if (level > 1.0 - 1.0/2.0/size)
...
(* The following two values show that the problem above is fixed with the percentile definition.
 *) 
...
assumes "level > 1 - 1/2/(length (values :: (real list)))"
        "level \<le> 1"
        "length values > 1"
(* OLD: shows "percentile_sorted values level \<le> values ! ((length values) -1)" *)
        "sorted values"
shows "percentile_sorted values (level :: real) \<le> 
       values ! ((length values) -1) + (maximum (diff_list values))/2.0"
proof -
have "percentile_sorted values level = percentile_sorted_above values level" using assms
      by (simp add: percentile_sorted_def)
then have f1: "percentile_sorted values level = 
               (values ! ((length values) - 1)) +
               ((maximum (diff_list values)) * (level - 1.0 + 1.0/2.0/(real (length values))) * (length values))"
by (simp add: percentile_sorted_def percentile_sorted_above_def)
have "(level - 1.0 + 1.0/2.0/(real (length values))) \<le> 1.0/2.0/(real (length values))" using assms
           by simp
then have "((maximum (diff_list values)) * (level - 1.0 + 1.0/2.0/(real (length values)))) \<le>
      ((maximum (diff_list values))* 1.0/2.0/(real (length values)))" using assms diff_list_max_non_negative
      by (metis ordered_comm_semiring_class.comm_mult_left_mono times_divide_eq_right)
then have "values ! ((length values) -1) + ((maximum (diff_list values)) * (level - 1.0 + 1.0/2.0/(real (length values)))) \<le>
       values ! ((length values) -1) + ((maximum (diff_list values))* 1.0/2.0/(real (length values)))" by auto
then show ?thesis 
    by (smt assms(3) f1 le_less_trans of_nat_0_less_iff pos_divide_le_eq pos_divide_less_eq times_divide_eq_right zero_le_one)
qed
...
def lowerValue \<equiv> "values ! (nat (i-1))"
def upperValue  \<equiv> "values ! (nat i)"
...
have f20: "lowerValue \<le> upperValue" using lowerValue_def upperValue_def using f19 by blast
have f21: "lowerValue +  diff * (upperValue - lowerValue) \<le> upperValue" using f11 f20 
          linear_interpolation by auto
have f22: "percentile_sorted_below values level = lowerValue + diff * (upperValue - lowerValue)" 
    using percentile_sorted_below_def size_def lower_def upper_def lowerValue_def upperValue_def 
          diff_def i_def by (metis (no_types, hide_lams))
have f23: "percentile_sorted values level = lowerValue + diff * (upperValue - lowerValue)"
     using f22 assms(2) percentile_sorted_def by simp 
have f24: "percentile_sorted values level \<le> upperValue" using f21 f23  by simp 
have f25: "i < size" using f18 by linarith
show ?thesis using f24 f25 any_smaller_sorted_last_var upperValue_def
...
(* OLD: shows "percentile_sorted values level \<le> values ! ((length values) - 1)" *)
shows   "percentile_sorted values (level :: real) \<le> 
       values ! ((length values) -1) + (maximum (diff_list values))/2.0*(real (length values))"
      using percentile_sorted_max_above percentile_sorted_max_below
proof -
      have "values ! ((length values) - 1) \<le> values ! ((length values) - 1) +  (maximum (diff_list values))/2.0*(real (length values))"
            using assms(3) assms(4) diff_list_max_non_negative by auto
      then have "level \<le> 1 - (1/2)/(length values) \<longrightarrow> ?thesis" using percentile_sorted_max_below assms
by fastforce
      then show ?thesis using assms  percentile_sorted_max_above 
      by (smt \<open>values ! (length values - 1) \<le> values ! (length values - 1) + 
         maximum (diff_list values) / (20 / 10) * real (length values)\<close> 
        le_divide_eq_1_pos nat_less_real_le nonzero_mult_divide_cancel_left of_nat_1)
qed
...
proof -
      ...
      have f4: "percentile_sorted_below values (1 - (1/2)/(length values)) = 
                 lowerValue +  \<alpha> * (upperValue - lowerValue)"
           by (metis \<alpha>_def i_def lowerValue_def lower_def percentile_sorted_below_def 
               upperValue_def)
      ...
      have f10: "percentile_sorted_below values (1 - (1/2)/(length values)) = upperValue" 
           using f4 f9 by simp
      show ?thesis using f10 upperValue_def f2 percentile_sorted_def by auto
qed
...
assumes "0 \<le> (level :: real)"
        "level \<le> 1"
        "length values > 1"
(* OLD : shows "percentile values level \<le> maximum values" *)
shows "percentile values level \<le> maximum values + (maximum (diff_list (sort values)))/2.0*(real (length values))"
       by (metis assms(1) assms(2) assms(3) length_invariance_sort max_equal_max_sorted_var 
           max_sorted_last_var percentile_def percentile_sorted_upper_bound sorted_sort)
 ...
 *)
(*** NEEDED TO BE DELETED SINCE THE COROLLARY IS NO LONGER TRUE AND THERE IS NO OBVIOUS REPLACEMENT ***)
...
def lowerValue1 \<equiv> "values ! (nat (i1-1))"
def lowerValue2 \<equiv> "values ! (nat (i2-1))"
def upperValue1  \<equiv> "values ! (nat i1)"
def upperValue2  \<equiv> "values ! (nat i2)"
...
have f15: "percentile_sorted_below values p1 = lowerValue1 + diff1 * (upperValue1 - lowerValue1)" 
         using  percentile_sorted_below_def size_def lower1_def upper1_def lowerValue1_def 
                upperValue1_def  diff1_def i1_def by metis 
have f16: "percentile_sorted_below values p2 = lowerValue2 + diff2 * (upperValue2 - lowerValue2)" 
         using  percentile_sorted_below_def size_def lower2_def upper2_def lowerValue2_def 
                upperValue2_def  diff2_def i2_def by metis 
{assume f17_1: "i1 = i2" 
 ...
 have   f17_4: "lowerValue1 = lowerValue2" using f17_1 lowerValue1_def lowerValue2_def by blast
 have   f17_5: "upperValue1 = upperValue2" using f17_1 upperValue1_def upperValue2_def by blast
 have   f17_6: "diff1 \<le> diff2"
        using assms(2) diff1_def diff2_def f3 f17_2 by auto
 have   f17_7: "lowerValue1 \<le> upperValue1" using assms(4) lowerValue1_def upperValue1_def sort_next
...
have f18_10: "lowerValue1 \<le> upperValue1" using assms(4) lowerValue1_def upperValue1_def sort_next
...
have f18_11: "lowerValue1 + diff1 * (upperValue1 - lowerValue1) \<le> upperValue1" 
...
have f18_12:  "percentile_sorted_below values p1 \<le> values ! (nat i1)" 
              using f15 f18_11 upperValue1_def by auto
...
have f18_17: "diff2 \<le> 1" using ceiling_upper f18_16 
           using add_divide_distrib ceiling_le_iff divide_self_if by linarith 
have f18_18: "lowerValue2 \<le> upperValue2"  using assms(4) lowerValue2_def upperValue2_def sort_next
              using assms(5) f14 of_nat_less_imp_less size_def by blast
have f18_19: "lowerValue2 \<le> lowerValue2 + diff2 * (upperValue2 - lowerValue2)" 
...
have f18_20:  "values ! (nat (i2 -1)) \<le> percentile_sorted_below values p2" 
              using f16 f18_19 lowerValue2_def by auto
...
shows "percentile_sorted values p1 \<le> percentile_sorted values p2"
proof -
def p_bar \<equiv> "1.0 - 1.0/2.0/(real (length values))"

have f0: "percentile_sorted values p2 = percentile_sorted_above values p2" using percentile_sorted_def assms(3) 
          by simp
then have f1: "percentile_sorted values p2 =  values ! ((length values) -1) + 
            (maximum (diff_list values)) * (p2 - p_bar) * (real (length values))" 
          using percentile_sorted_def percentile_sorted_above_def assms(3) p_bar_def
          using linordered_field_class.sign_simps(24) mult.assoc by auto
have f2: "p1 \<le>  p_bar \<or> p1 >  p_bar" by auto
{assume f3_1: "p1 \<le> p_bar"
 def size \<equiv> "real (length values)"
 def i1 \<equiv> "ceiling (size * p1 - 0.5)"
 def lower1 \<equiv> "(i1 - 0.5) / size"
 def upper1 \<equiv> "(i1 + 0.5) / size" 
...
have f3_5: "p_bar * (length values) = (length values) - 1/2" 
           using f3_4 p_bar_def by (simp add: Groups.mult_ac(2) right_diff_distrib)
have f3_6: "percentile_sorted_below values p1 = lowerValue1 + diff1 * (upperValue1 - lowerValue1)" 
         using  percentile_sorted_below_def size_def lower1_def upper1_def lowerValue1_def 
                upperValue1_def  diff1_def i1_def by metis 
...
have f3_13: "size * p1 \<le> size * p_bar"
           by (smt f3_1 f3_2 real_mult_le_cancel_iff2 size_def)
have f3_14: "size * p1 \<le> p_bar * size" using f3_13 by (simp add: mult.commute)
...
have f3_19: "lowerValue1 \<le> upperValue1" 
            using f3_12 f3_18 assms(4) lowerValue1_def upperValue1_def sorted_index
            by blast
have f3_20: "lowerValue1 + diff1 * (upperValue1 - lowerValue1) \<le> upperValue1" 
             using f3_9 f3_19 linear_interpolation
             by (smt mult_less_cancel_right1)
have f3_21: "percentile_sorted_below values p1 \<le> values ! (nat i1)" 
                using f3_6 f3_20 upperValue1_def by auto
have f3_22: "percentile_sorted_below values p1 \<le> values ! ((length values) - 1)"
            using assms(4) any_smaller_sorted_last_var
            assms(1) assms(5) f3_1 percentile_sorted_def percentile_sorted_max_below p_bar_def by auto
have f3_23: "p2 - p_bar \<ge> 0" using assms(3) p_bar_def by linarith
then have "percentile_sorted_below values p1 \<le> values ! ((length values) - 1) + 
            (maximum (diff_list values)) * (p2 - p_bar) * (real (length values))" using f3_22 p_bar_def diff_list_max_non_negative assms(5)
             by (smt assms(4) mult_nonneg_nonneg of_nat_0_le_iff)
then have "percentile_sorted_below values p1 \<le> percentile_sorted values p2" using f1 by auto
then have f3_24: ?thesis using percentile_sorted_def f3_1 p_bar_def by auto   
}
have f3: "p1 \<le>  p_bar \<Longrightarrow> 
          percentile_sorted values p1 \<le> percentile_sorted values p2"
          using \<open>p1 \<le> p_bar \<Longrightarrow> 
          percentile_sorted values p1 \<le> percentile_sorted values p2\<close> by blast
{assume f4_1: "p1 > p_bar"
 have   f4_2: "percentile_sorted values p1 = values ! ((length values) - 1) + 
                   (maximum (diff_list values)) * (p1 - p_bar) * (length values)"
        using f4_1 percentile_sorted_def percentile_sorted_above_def p_bar_def by auto
 then have   f4_3: "(maximum (diff_list values)) * (p1 - p_bar) * (length values) \<le>
               (maximum (diff_list values)) * (p2 - p_bar) * (length values)"
          using assms 
          by (smt diff_list_max_non_negative le_less_trans of_nat_0_less_iff 
             ordered_comm_semiring_class.comm_mult_left_mono real_mult_le_cancel_iff1 zero_le_one)
 then have   f4_4: ?thesis using f1 f4_2 by linarith
}
have  f4: "p1 >  p_bar \<Longrightarrow> 
           percentile_sorted values p1 \<le> percentile_sorted values p2"
          using \<open>p_bar < p1 \<Longrightarrow> 
          percentile_sorted values p1 \<le> percentile_sorted values p2\<close> by blast
show ?thesis using f2 f3 f4 by blast
qed

(* The lemma states that the percentile_sorted function is monotonic in the thresholds. 
 *)
...
assumes   "0 \<le> p1"
          ...
          "length values >1"
shows "percentile_sorted values p1 \<le> percentile_sorted values p2"
       by (smt assms(1) assms(2) assms(3) assms(4) assms(5) percentile_sorted_monotonic_above 
           percentile_sorted_monotonic_below)
 ...
 * confidence level 0.
 *)
(*** NEEDED TO ADD 2nd CONDITION ***)
corollary percentile_lower_bound:
assumes "0 \<le> level"
        "level \<le> 1"
        "length values > 1"
shows "percentile values 0 \<le> percentile values level"
using percentile_monotonic assms(1) assms(2) assms(3) by blast

(* In this section we show that multiplying the list of values with a non-negative scalar
...
(* An element of a list scaled by t at a particular position is the orignal element at the same
...
(*** NEW AUXILIARY LEMMAS NEEDED ***)

lemma diff_list_scale_aux:
   assumes "\<forall> (x1::real). \<forall> (y1::real). \<forall> (xs1:: (real list)). (length xs1 = n \<longrightarrow> diff_list (scale t (x1#y1#xs1)) = scale t (diff_list (x1#y1#xs1)))"
           "length xs = Suc n"
   shows   "diff_list (scale t (x#y#xs)) = scale t (diff_list (x#y#xs))"
   proof -
    have "length (tl xs) = n" using assms(2) by simp
    then have "diff_list (scale t (y#(hd xs)#(tl xs))) = scale t (diff_list (y#(hd xs)#(tl xs)))" using assms(1) by blast
    then have "diff_list (scale t (x#y#(hd xs)#(tl xs))) = scale t (diff_list (x#y#(hd xs)#(tl xs)))"
          by (simp add: linordered_field_class.sign_simps(38))
   then have "diff_list (scale t (x#y#xs)) = scale t (diff_list (x#y#xs))"
        by (metis One_nat_def \<open>length (tl xs) = n\<close> assms(2) hd_Cons_tl list.sel(2) list.size(3) zero_neq_one)
   then show ?thesis  by blast
qed 

lemma diff_list_scale:
  (* fixes xs :: "real list" *)
  shows "\<forall>x.\<forall>y.\<forall>xs. length xs = n \<longrightarrow> diff_list (scale t (x#y#xs)) = scale t (diff_list (x#y#xs))"
  proof (induct n)
   (* print_cases *)
  case 0
   then show ?case 
   ...
   next
  case (Suc n)
  {assume assm: "\<forall> x. \<forall>y. \<forall>xs. length xs = n \<longrightarrow> diff_list (scale t (x#y#xs)) = scale t (diff_list (x#y#xs))"
    have "\<forall>x.\<forall>y.\<forall>xs. length xs = (Suc n) \<longrightarrow> diff_list (scale t (x#y#xs)) = scale t (diff_list (x#y#xs))" using diff_list_scale_aux
      using assm by blast
  }
   then show ?case using Suc.hyps by blast
qed 
...
lemma scale_0:
  shows "maximum (scale 0 (x # xs)) = 0" 
  proof (induct "length xs")
  case 0
   then show ?case by simp
   next
  case Suc
   then show ?case
      by (smt One_nat_def Suc_less_eq diff_Suc_1 le_less_trans length_Cons lessI 
         max_sorted_last_var mult_cancel_left right_diff_distrib scalar_pos scale_length 
          sorted_equals_nth_mono zero_less_Suc)
qed

lemma scale_maximum:
  fixes xs :: "real list"
  assumes "(t:: real) \<ge> 0"
  shows "maximum (scale t (x#xs)) = t * (maximum (x#xs))"
  proof -
  {assume a: "t > 0"
  have "maximum (scale t (x#xs)) = maximum (sort (scale t (x#xs)))"
       using max_equal_max_sorted by auto
  then have "maximum (scale t (x#xs)) = (sort (scale t (x#xs))) ! ((length (scale t (x#xs))) -1)"
         by (smt diff_is_0_eq length_greater_0_conv length_invariance_sort length_tl 
             less_numeral_extra(3) list.sel(3) max_sorted_last max_sorted_last_var not_le 
             scale_simps(2) sorted_single sorted_sort sorted_sort_id)
  then have f1: "maximum (scale t (x#xs)) = (sort (scale t (x#xs))) ! (length (x#xs) -1)"
          by (simp add: scale_length)
  have "(scale t  (sort ys)) = sort (scale t ys)" using a scale_sort[of t ys] by blast

  then have "(sort (scale t (x#xs))) ! (length (x#xs) -1)  = t * (sort (x#xs)) !  (length (x#xs) -1)" using a scale_sort
        by (metis diff_less length_greater_0_conv length_invariance_sort list.distinct(1) scalar_pos zero_less_one)
  then have "maximum (scale t (x#xs)) = t * (sort (x#xs)) !  (length (x#xs) -1)" using assms scale_sort f1 by auto
  then have ?thesis by (smt length_greater_0_conv length_invariance_sort length_tl list.sel(3) list.size(3) 
            max_equal_max_sorted max_sorted_last_var maximum.simps(1) nth_Cons_0 sorted_single 
           sorted_sort sorted_sort_id zero_less_diff)
  }
  have f1: "t > 0 \<longrightarrow> ?thesis"
       using \<open>0 < t \<Longrightarrow> maximum (scale t (x # xs)) = t * maximum (x # xs)\<close> by blast
 
  {assume b: "t = 0"  
    then have "maximum (scale t (x # xs)) = 0" using scale_0 by auto
    then have ?thesis by (simp add: b)
  }
  have f2: "t = 0 \<longrightarrow> ?thesis" 
    using \<open>t = 0 \<Longrightarrow> maximum (scale t (x # xs)) = t * maximum (x # xs)\<close> by blast
 then show ?thesis using assms f1 f2 by linarith
qed

lemma diff_list_scale_maximum_aux:
  fixes xs :: "real list"
  assumes t: "(t:: real) \<ge> 0"
  shows  "maximum (diff_list (scale t (x#y#xs))) = t * (maximum (diff_list (x#y#xs)))"
  proof -
  have f1: "maximum (diff_list (scale t (x#y#xs))) = maximum (scale t (diff_list (x#y#xs)))" using 
        diff_list_scale by (simp add: t)
  have "maximum (scale t (diff_list (x#y#xs))) = t * (maximum (diff_list (x#y#xs)))" using scale_maximum
proof -
  have f1: "\<And>r ra rs. diff_list (r # ra # rs) \<noteq> [] \<or> length (r # rs) = 0"
    by (metis diff_list_length_aux length_0_conv)
  have f2: "\<forall>rs. \<exists>r ra rb rsa rc rd. (r - ra) # diff_list (r # rb # rsa) = diff_list rs \<or> [rc - rd] = diff_list rs \<or> diff_list rs = []"
    by (metis diff_list.elims)
  have "diff_list (x # y # xs) = [] \<longrightarrow> maximum (scale t (diff_list (x # y # xs))) = t * maximum (diff_list (x # y # xs))"
    using f1 by blast
  then show ?thesis
    using f2 by (metis (no_types) scale_maximum t)
qed
  then show ?thesis using f1 by simp
qed

lemma diff_list_scale_maximum:
  fixes xs :: "real list"
  assumes t: "(t:: real) \<ge> 0"
          "length xs > 1"
  shows  "maximum (diff_list (scale t xs)) = t * (maximum (diff_list xs))"
  proof -
  have "\<exists> (x::real). \<exists> (y::real). \<exists> (ys:: real list). xs = x#y#ys" using assms(2) 
       by (metis Percentile2.member.simps(1) diff_list.elims diff_list_lemma le_numeral_extra(4))  
  then show ?thesis using diff_list_scale_maximum_aux using t(1) by blast
qed
...
proof (induct "length xs")
   ...
   then show ?case
   proof -
   have "diff_list (x#y#[]) = (y-x)#[]" by simp
   then have f1: "(maximum (diff_list (x#y#[]))) = y-x" by simp
   have "scale t (x#y#[]) = (t*x)#(t*y)#[]" by simp
   then have "diff_list (scale t (x#y#[])) = ((t*y) - (t*x)) # []" by simp
   then have "maximum (diff_list (scale t (x#y#[]))) = (t*y) - (t*x)" by simp
   then have "maximum (diff_list (scale t (x#y#[]))) = t * (y - x)"
        by (simp add: right_diff_distrib)
   then show ?case using f1 using "0.hyps" t(2) by linarith
   qed
   ...
   next 
    ...
    then show ?case
    proof -
    have "maximum (diff_list (scale t (x # y # xs))) = t * maximum (diff_list (x # y # xs))
*)
...
lemma homogeneity_below:
   assumes t: "(t :: real) > 0"
    ...
    shows "percentile_sorted_below (scale t values) level = 
     ...
     def lowerValue \<equiv> "values ! (nat (i -1))"
     def tlowerValue \<equiv> "tvalues ! (nat (ti -1))"
     def upperValue \<equiv> "values ! (nat i)"
     def tupperValue \<equiv> "tvalues ! (nat ti)"
     def lower \<equiv> "(i - 0.5) / size"
     ...
     def lpercentile \<equiv> "lowerValue + alpha * (upperValue - lowerValue) "
     def tlpercentile \<equiv> "tlowerValue + talpha * (tupperValue - tlowerValue)"
     have f1: "size > 0" 
          using assms(4) size_def less_trans of_nat_0_less_iff of_nat_le_0_iff by auto
     have f111: "real (length values) > 0" using assms(4) by auto
     have f112: "level \<le> ((real (length values)) - 0.5)/(real (length values))" 
          using f111 assms(3)
          by (simp add: diff_divide_distrib) 
     have f11: "level * (real (length values)) \<le> (real (length values)) - 0.5" 
          using f112 assms(3) f111 pos_le_divide_eq by blast
     have f12: "(real (length values)) * level - 0.5 \<le> (real (length values)) - 1" 
          using f11 by (simp add: add_divide_distrib mult.commute)
     have f2: "ceiling ((real (length values)) * level - 0.5) < real (length values)" 
             using f12 le_ceiling_iff by linarith
     have f3: "i < size" using f2 i_def size_def by (simp add: less_ceiling_iff)            
     have f4: "upper - lower = (i + 0.5)/size - (i - 0.5)/size" using upper_def lower_def by blast     
     have f5: " (i + 0.5)/size - (i - 0.5)/size = (i+0.5 - (i-0.5))/size" 
           by (metis diff_divide_distrib)
     have f6: "upper - lower = 1/size" using f4 f5
           by (simp add: add_divide_distrib divide_less_cancel)
     have f7: "alpha = (level - lower) * size" using alpha_def f6 by simp 
     have f8: "lowerValue + (level - lower) * size * (upperValue - lowerValue) = 
              percentile_sorted_below values level"
          using assms(4) percentile_sorted_below_def lowerValue_def upperValue_def
                 lower_def i_def size_def by (metis (no_types, hide_lams))
     have f9: "lpercentile = lowerValue + alpha * (upperValue - lowerValue)" 
          using lpercentile_def by blast
     have f10: "percentile_sorted_below values level = lpercentile" using f7 f8 f9 by auto
     have f11: "size = tsize"
          using size_def tsize_def scale_length tvalues_def by simp
     have f12: "length values = length (scale t values)"  using scale_length by auto
     have f13: "i = ti"
         using f11 i_def ti_def by blast
     have f14: "tvalues = scale t values"
           using assms scale_sort tvalues_def by blast
     have f15: "nat i < length values" using f1 using f3 nat_eq_iff nat_less_iff size_def by linarith
     have f16: "(nat (i-1)) < length values" using f15
          using nat_le_0 nat_less_iff by linarith
     have f17: "((nat (i -1))  < length values) \<longrightarrow> 
                 (scale t values) ! (nat (i-1)) = t * (values ! (nat (i-1)))" 
          using scalar_pos by auto
     have f18: "(scale t values) ! (nat (i-1)) = t * (values ! (nat (i-1)))"
        using f16 scalar_pos by blast
     have f19: "tlowerValue = t * lowerValue"
        using f13 f18 lowerValue_def tlowerValue_def tvalues_def by blast
     have f20: "(scale t values) ! (nat i) = t * (values ! (nat i))"
        using f15 scalar_pos by blast
     have f21: "tupperValue = t * upperValue" 
                using f13 f20 tupperValue_def upperValue_def tvalues_def by blast
     have f22: "tupperValue - tlowerValue = t * (upperValue - lowerValue)"
        using f19 f21 by (metis (mono_tags, lifting) right_diff_distrib')
     have f23: "alpha = talpha"
         using alpha_def talpha_def lower_def tlower_def upper_def tupper_def f11 f13 by blast
     have f24: "talpha * (tupperValue - tlowerValue) =
                 t *  alpha * (upperValue - lowerValue)"
        using f22 f23 by auto
     have f25:  "tlowerValue + talpha * (tupperValue - tlowerValue)  =
                 t *  (lowerValue + alpha * (upperValue - lowerValue))"
           using f19 f24
           by (metis (no_types, hide_lams) distrib_right mult.assoc mult.commute)
     have f26: "tlpercentile = t * lpercentile"
           using f25 lpercentile_def tlpercentile_def by metis
     have f28: "tlowerValue + (level - tlower) * tsize * (tupperValue - tlowerValue) = 
              percentile_sorted_below (scale t values) level"
          using assms(4) percentile_sorted_below_def tlowerValue_def tupperValue_def
                 tlower_def ti_def tsize_def by (metis (full_types) f14)
     have f29: "percentile_sorted_below (scale t values) level = tlpercentile"
         using tlpercentile_def f28 talpha_def
         using f11 f13 f23 f7 lower_def tlower_def by auto
     show ?thesis using f29 f26 f10 by blast
qed
  ...
  assumes t: "(t :: real) > 0"
    ...
    {assume f2_1: "level \<le> p_bar"
     have f2_2:  "percentile_sorted_below (scale t values) level = 
          t * percentile_sorted_below values level" using f2_1 homogeneity_below
          using t(1) t(2) t(4) p_bar_def
          by (simp add: 
             linordered_field_class.sign_simps(24) nonzero_eq_divide_eq real_sum_of_halves)
     have ?thesis using f2_1 f2_2 percentile_sorted_def
          by (simp add: p_bar_def scale_length)
   }
   ...
   {assume f3_1: "level > p_bar"
      then have "percentile_sorted values level = percentile_sorted_above values level"
           by (simp add: p_bar_def percentile_sorted_def)
      then have f3_2: "percentile_sorted_above values level = values ! (size -1) + 
                  (maximum (diff_list values)) * (real size)  * (level - p_bar)"
            using size_def
            percentile_sorted_above_def p_bar_def assms using divide_self_if by auto
      then have f3_3: "percentile_sorted (scale t values) level = 
                 (scale t values) ! (size -1) + maximum (diff_list (scale t values))  * (real size) * (level - p_bar)" 
            using f3_1 percentile_sorted_def[of "scale t values" level]
            by (simp add: p_bar_def percentile_sorted_above_def scale_length size_def)
      have "(scale t values) ! (size -1) = t * (values ! (size -1))"
            using scalar_pos size_def t(4) by auto
      then have f3_4: "percentile_sorted (scale t values) level = t * (values ! (size -1)) +
                   maximum (diff_list (scale t values)) * (level - p_bar) * (real size)"
          using f3_3 by auto
      then have "percentile_sorted (scale t values) level = t * (values ! (size -1)) +
                  t * (maximum (diff_list values)) * (level - p_bar) * (real size)"
          using diff_list_scale_maximum size_def t(4)
                 by (metis divide_eq_0_iff divide_le_eq_1_pos t(1) zero_le_one)
       then have ?thesis
            by (simp add: \<open>percentile_sorted values level = percentile_sorted_above values level\<close>
                 distrib_left f3_2)
  }
...
lemma upperbound_sorted_small_step_below:
  ...
  proof - 
   ...
   def lowerValue  \<equiv> "sorted_values ! (nat (i-1))"
   def upperValue  \<equiv> "sorted_values ! (nat i)"
   ...
   have f6: "percentile_sorted_below sorted_values p1 = 
             lowerValue +  diff1 * (upperValue - lowerValue)" 
        using  i_def lower_def upper_def lowerValue_def upperValue_def diff1_def
   ...
   have f11: "percentile_sorted_below sorted_values p2 = 
              lowerValue +  diff2 * (upperValue - lowerValue)" 
        using  f10 lower_def upper_def lowerValue_def upperValue_def diff2_def
   ...
   have f12: "(percentile_sorted_below sorted_values p2) - 
             ...
             (diff2 - diff1) * (upperValue - lowerValue)" using f6 f11
   ...
   have f18: "member (upperValue - lowerValue) (diff_list sorted_values)" 
             using f14 f17 lowerValue_def upperValue_def
        by auto
   have f19: "upperValue - lowerValue \<le> max_jump" 
             using upperValue_def lowerValue_def assms(8) assms(3) assms(5) maximum_lemma 
   ...
   have f21: "(percentile_sorted_below sorted_values p2) - 
              (percentile_sorted_below sorted_values p1) = 
              (p2 - p1) * size * (upperValue - lowerValue)" using f12 f20 by simp
...
lemma upperbound_boundary:
   ...
   have f1: "percentile_sorted_below sorted_values p = 
              lowerValue +  diff * (upperValue - lowerValue)" 
        by (metis (no_types) diff_def i_def lowerValue_def lower_def percentile_sorted_below_def size_def upperValue_def)
   have "p \<le> 1 - (1/2/(real size))" using size_def assms(4) 
        by (simp add: add_divide_distrib real_sum_of_halves)
   then have "percentile_sorted sorted_values p = percentile_sorted_below sorted_values p" using assms(4) percentile_sorted_def 
        by (simp add: size_def)
   then have f2: "percentile_sorted sorted_values p = lowerValue +  diff * (upperValue - lowerValue)" using assms(4)
        using f1 by auto
   have f3: "real size > 0" using size_def assms(2) using le_less_trans of_nat_0_less_iff zero_le_one by linarith
   have "p = 1 - 1/2/(real size)" by (simp add: assms(4) size_def)
   then have "(real size) * p = (real size) * (1 - 1/2/(real size))" using size_def assms(2) by auto
   then have "(real size) * p = (real size) - (real size) * 1/2/(real size)" by (simp add: right_diff_distrib)
...
proof -
  ...
  have "- 1 * (1 / 2) + real_of_int (int size) + 1 / 2 = real_of_int (int size)"
    by auto
  then have f2: "1 / 2 / real_of_int (int size) + (- 1 * (1 / 2) + real_of_int (int size)) / real_of_int (int size) + - 1 * (real_of_int (int size) / real_of_int (int size)) = 0"
...
qed
   then have "upper - lower = 1/(real size)" using upper_def lower_def
        by (simp add: \<open>p = 1 - 1 / 2 / real size\<close> add_divide_distrib assms(2) assms(4) diff_divide_distrib f3 real_sum_of_halves size_def)
   then have f5: "diff = 1" using f4 using diff_def f3 nonzero_eq_divide_eq by auto
   then have "percentile_sorted sorted_values p = upperValue" using f2 by auto
   then show ?thesis by (simp add: \<open>i = int (size - 1)\<close> size_def upperValue_def)
qed
...
lemma lowerbound_boundary:
   ...
   then have f0: "(percentile_sorted sorted_values p) = percentile_sorted_below sorted_values p" 
    ...
    have f3: "lowerValue = sorted_values ! 0" using f2 lowerValue_def by auto
    have f4: "upperValue = sorted_values ! 0" using f1 upperValue_def by auto
    have f5: "lowerValue + \<alpha> * (upperValue - lowerValue) = sorted_values ! 0" using f3 f4 by auto
    have f6: "percentile_sorted_below sorted_values p = lowerValue + \<alpha> * (upperValue - lowerValue)"
        using size_def i_def lower_def upper_def lowerValue_def upperValue_def \<alpha>_def
...
lemma upperbound_sorted_small_step:
   ...
   def p_bar \<equiv> "1 - 0.5/(real (length sorted_values))"
   {assume "p2 \<le> p_bar \<and> (real (length sorted_values)) * p2 > 0.5"
    then have ?thesis using upperbound_sorted_small_step_below
          assm_length_gt_1 ex_n assm_p1_p2 assm_values_sorted 
          max_jump_def max_slope_def percentile_sorted_def p_bar_def by auto
   }
   have f1: "p2 \<le> p_bar \<and> (real (length sorted_values)) * p2 > 0.5 \<longrightarrow>
         ?thesis"
         using \<open>p2 \<le> p_bar \<and> 
   ...
   {assume a2: "p2 > p_bar"
    then have"percentile_sorted sorted_values p2 = 
               sorted_values!((length sorted_values) - 1) + 
                  (maximum (diff_list sorted_values)) * (p2 - p_bar) * (real (length sorted_values))"
           using assm_values_sorted percentile_sorted_def  percentile_sorted_above_def p_bar_def by auto
     ...
     {assume "p1 > p_bar"
      then have "percentile_sorted sorted_values p1 = sorted_values!((length sorted_values) - 1) + 
                  (maximum (diff_list sorted_values)) * (p1 - p_bar) * (real (length sorted_values))"
       using assm_values_sorted percentile_sorted_def  percentile_sorted_above_def p_bar_def by auto
       then have "(percentile_sorted sorted_values p2) - (percentile_sorted sorted_values p1) =
               (maximum (diff_list sorted_values)) * (p2 - p_bar) * (real (length sorted_values)) -
               (maximum (diff_list sorted_values)) * (p1 - p_bar) * (real (length sorted_values))"
           using \<open>percentile_sorted sorted_values p2 = sorted_values ! (length sorted_values - 1) + 
                  maximum (diff_list sorted_values) * (p2 - p_bar) 
                * real (length sorted_values)\<close> by auto
      then have "(percentile_sorted sorted_values p2) - (percentile_sorted sorted_values p1) = 
                (maximum (diff_list sorted_values)) * (p2 - p1) * (real (length sorted_values))" using diff_divide_distrib distrib_left
       
proof -
  have "\<And>r ra rb. (r::real) * ra + - (r * rb) = r * (ra + - rb)"
    by (simp add: right_diff_distrib)
  then have "real (length sorted_values) * (max_jump * (p2 - p_bar)) - 
            (real (length sorted_values) * (max_jump * (p1 - p_bar))) = 
              real (length sorted_values) * (max_jump * (p2 + - p1))"
    by simp
  then show ?thesis
    by (metis (no_types) \<open>percentile_sorted sorted_values p2 - percentile_sorted sorted_values p1 = 
         maximum (diff_list sorted_values) * (p2 - p_bar) * real (length sorted_values) - 
         maximum (diff_list sorted_values) * (p1 - p_bar) * real (length sorted_values)\<close> 
         add_uminus_conv_diff linordered_field_class.sign_simps(24) 
         max_jump_def)
qed
      then have ?thesis by (simp add: max_jump_def max_slope_def)
      }
      have "p1 > p_bar \<longrightarrow> ?thesis"
            using \<open>p_bar < p1 \<Longrightarrow> 
                    percentile_sorted sorted_values p2 - percentile_sorted sorted_values p1 \<le> 
                  (p2 - p1) * max_slope\<close> by blast
      ...
      {assume "p1 \<le>  p_bar"
       ...
       have f1: "(percentile_sorted sorted_values 
                    p_bar) =  sorted_values ! ((length sorted_values -1))" 
             using assm_length_gt_1 assm_values_sorted upperbound_boundary p_bar_def by blast
       have f2: "real (length sorted_values) > 1" using assm_length_gt_1 by auto
       then have "p_bar > 0" using divide_less_eq_1_pos p_bar_def by force
       then have f3: "(real (length sorted_values)) * p_bar > 
                 p_bar" using f2 divide_le_eq_1_pos pos_divide_less_eq by auto
       have "p_bar > 0.5" using assm_length_gt_1 divide_less_eq_1_pos f2 p_bar_def by force
       then have f4: "(real (length sorted_values)) * p_bar > 0.5" using f3 by linarith
       have f5: "(percentile_sorted_below sorted_values 
            ...
            using upperbound_sorted_small_step_below[of concl:  sorted_values "p_bar" p1] assms
          f4 \<open>p_bar < p2\<close> \<open>p1 \<le> p_bar\<close> p_bar_def by auto
       have "(percentile_sorted sorted_values p_bar)
                     - (percentile_sorted sorted_values p1) =
               (percentile_sorted_below sorted_values p_bar)
                     - (percentile_sorted sorted_values p1)"
                     using percentile_sorted_def assms p_bar_def
             by (simp add: add_divide_distrib real_sum_of_halves)
       then have f6: "(percentile_sorted sorted_values 
                    p_bar) - (percentile_sorted sorted_values p1)
                     \<le> 
                    (p_bar - p1) * max_slope" using f5 
                    \<open>p1 \<le> p_bar\<close> 
                assm_length_gt_1 assm_values_sorted assms(1) 
                diff_list_length diff_list_non_negative less_imp_le_nat max_slope_def 
                max_vs_first_element mult_nonneg_nonneg of_nat_0_le_iff percentile_sorted_above_def 
                percentile_sorted_def zero_less_diff p_bar_def by auto
       have "(percentile_sorted sorted_values  p2) = 
              sorted_values ! ((length sorted_values) - 1) +
              (maximum (diff_list sorted_values)) * (p2 - p_bar) * (length sorted_values)"
              using \<open>percentile_sorted sorted_values p2 = sorted_values ! (length sorted_values - 1) 
                    + maximum (diff_list sorted_values) * (p2 - p_bar) * real (length sorted_values)\<close> by blast
       then have f7: "percentile_sorted sorted_values p2 - (percentile_sorted sorted_values 
                    (1 - 0.5/(real (length sorted_values)))) =
                    (maximum (diff_list sorted_values)) * (p2 - p_bar) * (length sorted_values)"
                   using f1 p_bar_def by auto
       then have "(percentile_sorted sorted_values p2) - (percentile_sorted sorted_values p_bar)
                     = (p2 - p_bar) * max_slope" using p_bar_def
by (simp add: linordered_field_class.sign_simps(24) max_jump_def max_slope_def mult.assoc)
 
      then have "(percentile_sorted sorted_values p2) - (percentile_sorted sorted_values p1) \<le> 
                  (p2 - p1) * max_slope"  using f6 by (simp add: linordered_field_class.sign_simps(37))
     }
    then have ?thesis using \<open>p_bar < p2\<close>
         ...
         \<open>p_bar < p1 \<longrightarrow> 
         percentile_sorted sorted_values p2 - percentile_sorted sorted_values p1 \<le> 
         (p2 - p1) * max_slope\<close> by linarith
   }
   have f2: "p2 > p_bar \<longrightarrow> ?thesis"
        using \<open>p_bar < p2 \<Longrightarrow> 
            percentile_sorted sorted_values p2 - percentile_sorted sorted_values p1 \<le> 
            (p2 - p1) * max_slope\<close> by blast
   {assume "(real (length sorted_values)) * p2 \<le> 0.5"
    then have "(real (length sorted_values)) * p1 \<le> 0.5"
          by (smt assm_length_gt_1 assm_p1_p2 less_trans mult_less_cancel_left_pos of_nat_0_less_iff zero_less_one)
    then have "percentile_sorted sorted_values p2 = percentile_sorted sorted_values p1"
         using lowerbound_boundary[of concl: sorted_values p1] lowerbound_boundary[of concl: sorted_values p2] 
         using \<open>real (length sorted_values) * p2 \<le> 5 / 10\<close> assm_length_gt_1 assm_values_sorted by auto
...
lemma upperbound_sorted_bigger_step_below:
  ...
  assumes 
      ...
      (* assm_p1_big: "(real (length sorted_values)) * p1 > 0.5" and *)
  ...
  proof -
   ...
   def lower1 \<equiv> "(i1 - 0.5)/size"
   def upper1 \<equiv> "(i1 + 0.5)/size"
   def lowerValue1  \<equiv> "sorted_values ! (nat (i1-1))"
   def upperValue1  \<equiv> "sorted_values ! (nat i1)"
   def i2 \<equiv> "ceiling (size * p2 - 0.5)"
   def lower2 \<equiv> "(i2 - 0.5)/size"
   def upper2 \<equiv> "(i2 + 0.5)/size"
   def lowerValue2  \<equiv> "sorted_values ! (nat (i2-1))"
   def upperValue2  \<equiv> "sorted_values ! (nat i2)"
   def diff1 \<equiv> "(p1 - lower1) * size"
   def diff2 \<equiv> "(p2 - lower2) * size"
   have f1: "percentile_sorted_below sorted_values p1 = 
             lowerValue1 +  diff1 * (upperValue1 - lowerValue1)" 
        using  i1_def lower1_def upper1_def lowerValue1_def upperValue1_def diff1_def
        by (metis percentile_sorted_below_def size_def)
   have f2: "percentile_sorted_below sorted_values p2 = 
              lowerValue2 +  diff2 * (upperValue2 - lowerValue2)" 
        using i2_def lower2_def upper2_def lowerValue2_def upperValue2_def diff2_def
        by (metis percentile_sorted_below_def size_def)
   ...
   then have "percentile_sorted_below sorted_values p1 = upperValue1"
        by (simp add: f1)
   then have f21: "(percentile_sorted_below sorted_values p2) - 
              (percentile_sorted_below sorted_values p1) =  
              lowerValue2 +  diff2 * (upperValue2 - lowerValue2) - upperValue1" 
              by (simp add: f2)
   ...
   have "member (upperValue2 - lowerValue2) (diff_list sorted_values)" 
             using f24 f23 lowerValue2_def upperValue2_def assms(10) f13 by auto
   then have f25: "upperValue2 - lowerValue2 \<le> max_jump" 
             using upperValue2_def lowerValue2_def assms(8) assms(3) assms(5) maximum_lemma
   ...
   have "upperValue1 = lowerValue2"
        by (simp add: f12 lowerValue2_def upperValue1_def)
   then have f30:  "(percentile_sorted_below sorted_values p2) - 
              (percentile_sorted_below sorted_values p1) =   diff2 * (upperValue2 - lowerValue2)"
              using f21 by auto
...
lemma upperbound_one_step_var:
   fixes    max_jump max_slope :: real and
    ...
    assumes 
           ...
           assm_values_sorted: "sorted sorted_values" and
           max_slope_def: "max_slope = max_jump * (real (length sorted_values))" and
           level: "p = (\<lambda> m::nat. ((real 2)*(real m)-(real 1))/(real 2)/(real (length sorted_values)))" and
           n_prop: "0 < n \<and> n \<le> (length (sorted_values)) -1"
   shows "(* \<and>
            (length sorted_values) * (p n) > 0.5  *) 
          (percentile_sorted sorted_values (p (n+1))) - 
          (percentile_sorted sorted_values (p n))
           \<le> ((p (n+1)) - (p n)) * max_slope"
   proof -
   def p_bar \<equiv> "1.0 - 1.0/2.0/(real (length sorted_values))"
   {assume assm: "p (n+1) \<le> p_bar"
   ...
   then have "p n \<le> p_bar" using assm by auto
   then have "(percentile_sorted sorted_values (p (n+1))) - 
          (percentile_sorted sorted_values (p n)) = 
             (percentile_sorted_below sorted_values (p (n+1))) - 
          (percentile_sorted_below sorted_values (p n))" using percentile_sorted_def assm p_bar_def by simp
   have f1: "0 < p n" using n_prop level
         using assm_length_gt_1 divide_pos_pos mult_less_cancel_left2 nat_less_real_le 
         of_nat_0_le_iff of_nat_add one_add_one zero_less_one by auto
   have "(0 < n \<and> n \<le> (length (sorted_values)) -1 \<and>
         (p (n+1)) \<le> 1 - 0.5/(real (length sorted_values)))  \<longrightarrow> (percentile_sorted_below sorted_values (p (n+1))) - 
          (percentile_sorted_below sorted_values (p n)) \<le> ((p (n+1)) - (p n)) * max_slope" 
          using 
             upperbound_below_one_step_var[of concl: n sorted_values p max_slope] f1 assm n_prop
             using assm_length_gt_1 assm_values_sorted level max_jump_def max_slope_def by blast
   then have "(percentile_sorted_below sorted_values (p (n+1))) - 
          (percentile_sorted_below sorted_values (p n)) \<le> ((p (n+1)) - (p n)) * max_slope" using assm n_prop
         using add_divide_distrib divide_self_if p_bar_def real_sum_of_halves by linarith
   then have ?thesis
        using \<open>percentile_sorted sorted_values (p (n + 1)) - percentile_sorted sorted_values (p n) = 
                percentile_sorted_below sorted_values (p (n + 1)) - 
               percentile_sorted_below sorted_values (p n)\<close> by auto
   }   
   have f1: "p (n+1) \<le> p_bar \<longrightarrow> ?thesis"
        using \<open>p (n + 1) \<le> p_bar \<Longrightarrow> percentile_sorted sorted_values (p (n + 1)) - 
              percentile_sorted sorted_values (p n) \<le> (p (n + 1) - p n) * max_slope\<close> by blast
   {assume assm: "p (n+1) > p_bar"
      then have f2_1: "percentile_sorted_above sorted_values (p (n+1)) = 
                 sorted_values!((length sorted_values)-1) + max_slope * ((p (n+1)) - p_bar)" 
                 using add_divide_distrib percentile_sorted_above_def max_slope_def max_jump_def
                 p_bar_def by auto
      have "((real 2)*(real (n+1))-(real 1))/(real 2)/(real (length sorted_values)) > 
                  1 - (real 1)/(real 2)/(real (length sorted_values))" using assm level p_bar_def 
               by auto
      ...
      then have ?thesis using linorder_not_le n_prop by blast
  } 
   have f2: "p (n+1) > p_bar \<longrightarrow> ?thesis"
        using \<open>p_bar < p (n + 1) \<Longrightarrow> percentile_sorted sorted_values (p (n + 1)) - 
            percentile_sorted sorted_values (p n) \<le> (p (n + 1) - p n) * max_slope\<close> by blast
...
lemma upperbound_one_step_variant:
  ...
  assumes 
     ...
     * e.g., for [1, 3, 4, 9] the diff_list is [2,1,5] with maximum 5 *)
     ...
     max_slope_def: "max_slope = max_jump * (real (length sorted_values))" and
     level: "p = (\<lambda> m::nat. ((real 2)*(real m)-(real 1))/(real 2)/(real (length sorted_values)))"
  shows "\<forall> n :: nat.((0 < n \<and> n \<le> (length (sorted_values)) -1) (* \<and>
            (length sorted_values) * (p n) > 0.5) *)
...
lemma  upperbound_many_step_n_k:
   fixes    max_jump max_slope :: real and
    ...
    assumes 
           ...
           assm_values_sorted: "sorted sorted_values" and
           max_slope_def: "max_slope = max_jump * (real (length sorted_values))" and
           level: "p = (\<lambda> i::nat. ((real 2)*(real i)-(real 1))/(real 2)/(real (length sorted_values)))" and
           k_prop: "k \<le> m \<and> k > 0" and
           n_prop: "n \<le> k \<and> n > 0"
   shows (*"\<forall> i :: nat.((n \<le> i \<and> i \<le> k  (* \<and>
            (real (length sorted_values)) * (p i) > 0.5 *) 
          )
  \<longrightarrow> "*)  "(percentile_sorted sorted_values (p (k+1))) - (percentile_sorted sorted_values (p n)) 
   ...
   then have "(percentile_sorted sorted_values (p (i+1))) - 
          ...
          using assms upperbound_one_step_variant[of max_jump sorted_values max_slope] \<open>n \<le> i \<and> i \<le> k\<close> sorry (* to stay *)
   }
   ...
   have  "(\<Sum> i \<in> {n..k}. ((percentile_sorted sorted_values (p (i+1))) - 
          ...
          (percentile_sorted sorted_values (p n))" 
            ...
            f3 f4 by blast
   then show ?thesis using f2 sorry (* to stay *)
qed
...
lemma percentile_lipschitz_sorted_continuity:
   ...
   shows "(percentile_sorted sorted_values p2) - 
          (percentile_sorted sorted_values p1) \<le> 
                (p2 - p1) * max_slope " 
   proof -
   def p1n \<equiv> "(nat (floor ((real (length sorted_values)) * p1 - 1.0/2.0))) + 1" (* let p1n be the smallest i such that (p i) \<ge> p1 *)
   def p2n \<equiv> "(nat (floor ((real (length sorted_values)) * p2 - 1.0/2.0)))"   (* let p2n be the biggest i such that (p i) \<le> p2 *)
   ...
   have a111: "(real (nat (floor ((real (length sorted_values)) * p2 - 1.0/2.0))))- 1.0/2.0 \<le>  (real (length sorted_values)) * p2"
   ...
   have a12: "p1 \<le> p1r" sorry (* to stay *)
    ...
    then have "\<forall> (x::nat). \<forall> (y::nat). (p x) = (p y) \<longrightarrow> x = y" using f2 assm_length_gt_1 level by auto
   ...
   {assume a1: "p1r < p2r"
    ...
    have p2r_lower_aux: "(2*(real p2n)-1)/2/(real (length sorted_values)) \<le> p2r" using p2r_def p2n_def 
              by (simp add: level)

    have p2r_lower_add:  "(2*(real p2n)-1)/2/(real (length sorted_values)) \<noteq> p2r" using f5 sorry (* to stay *)
    then have p2r_lower: "(2*(real p2n)-1)/2/(real (length sorted_values)) < p2r" using p2r_lower_aux by auto
    have p2_upper: "p2 \<le> (2*(real p2n)+1)/2/(real (length sorted_values))" using level p2r_def p2r_lower by auto
    ...
    have f2: "(percentile_sorted sorted_values (p p2n)) - (percentile_sorted sorted_values (p p1n))
           ...
           using upperbound_many_step_n_k[of m sorted_values max_jump max_slope p p2n p1n] m_def f2_3 max_jump_def assm_length_gt_1 
              assm_values_sorted max_slope_def level f2_1 f5 of_nat_1 of_nat_numeral p2r_def p2r_lower_add sorry (* to stay *)

    have f3: "(real (length sorted_values)) * p1 \<le> (ceiling ((real (length sorted_values)) * p1 + 0.5))" using a12
          by linarith
    have "p1 \<le> (of_real (nat (ceiling ((real (length sorted_values)) * p1 - 0.5)))) /(real (length sorted_values))" using a1
          using level p2r_def p2r_lower by auto
    have p1_lower: "(2*(real p1n)-1)/2/(real (length sorted_values)) < p1" using level p2r_def p2r_lower_add by auto
    ...
    have f4: "(percentile_sorted sorted_values p1r) - (percentile_sorted sorted_values  p1)
           ...
           using upperbound_sorted_small_step[of max_jump sorted_values max_slope p1 p1r] 
             a12 max_jump_def max_slope_def a1 assm_length_gt_1
             a1 p1_lower p1r_upper assm_values_sorted f2_1 f5 less_imp_le_nat less_trans m_def by force
    ...
    {assume "\<not> (p1r < p2r)"
     ...
     then have "(floor ((real (length sorted_values)) * p1 - 1.0/2.0)) + 1 \<ge> 
                (floor ((real (length sorted_values)) * p2 - 1.0/2.0))" using  assm_p1_p2  assm_p2_lt_1 sorry (* stay *)
     then have "(floor ((real (length sorted_values)) * p1 + 1.0/2.0)) \<ge> 
                (floor ((real (length sorted_values)) * p2 - 1.0/2.0))" using  floor_upper_plus_1 by linarith 
     then have "(real (length sorted_values)) * p1 + 1.0/2.0 + 1 \<ge> 
                ((real (length sorted_values)) * p2 - 1.0/2.0)" using floor_semi_monotonic_inv by blast
     then have "(real (length sorted_values)) * p1 \<ge> (real (length sorted_values)) * p2 - 2" by auto
     then have ?thesis sorry (* to stay *)
    }
